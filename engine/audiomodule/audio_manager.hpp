	...
	public:
		...
		force_inline AudioID LoadSound(const std::string& _file, FMOD_MODE _mode) {
			if (!m_name_to_id.contains(_file)) {
				...
				m_audio_system->createSound(_file.c_str(), _mode, nullptr, &sound);
		...
		force_inline EmitterID CreateEmitter() {
		...
		[[nodiscard]] force_inline std::optional<std::reference_wrapper<AudioEmitter>> GetEmitter(const EmitterID& _id) {
			if (!m_emitters.contains(_id)) return std::nullopt;
			return std::ref(m_emitters[_id]);
		}
