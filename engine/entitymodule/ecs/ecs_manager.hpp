...
namespace ecs {
	...
	public:
		...
		force_inline std::optional<std::reference_wrapper<T>> AddComponent(detail::EntityID _id) {
		...
		force_inline void DeleteComponent(detail::EntityID _id) {
			ECS_ASSERT_IS_COMPONENT(T);
			auto& pool = GetOrCreatePool<T>();
			pool.Delete(_id);
		}

		template<detail::Component T>
		force_inline std::optional<std::reference_wrapper<T>> GetComponent(detail::EntityID _id) {
			ECS_ASSERT_IS_COMPONENT(T);
			auto& pool = GetOrCreatePool<T>();
			return pool.Get(_id);
		}
