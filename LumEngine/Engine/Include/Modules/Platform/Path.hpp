...
namespace lum {
	...
	public:
		...
		LUM_FORCEINLINE String ToString( ) const { return mPath.string( ); }
		LUM_FORCEINLINE String Extension( ) const { return mPath.extension( ).string(); }
		LUM_FORCEINLINE String Stem( ) const { return mPath.stem( ).string( ); }
		LUM_FORCEINLINE Path Filename( ) const { return mPath.filename( ); }
		LUM_FORCEINLINE Path ParentPath( ) const { return mPath.parent_path( ); }
		LUM_FORCEINLINE Path LexicallyNormal( ) const { return mPath.lexically_normal( ); }
		LUM_FORCEINLINE Path RelativePath( ) const { return mPath.relative_path( ); }
		LUM_FORCEINLINE bool IsAbsolute( ) const { return mPath.is_absolute( ); }
