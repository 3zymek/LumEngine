...
namespace lum::gl {
	...
	rhi::ShaderHandle	GL_Device::CreateShader	( const ShaderDescriptor& desc ) {
		if (m_shaders.DenseSize() >= MAX_SHADERS) {
			LUM_LOG_ERROR("Max shaders reached");
	...
	rhi::TextureHandle	GL_Device::CreateTexture2D(const TextureDescriptor& desc) {
		if (m_textures.DenseSize() >= MAX_TEXTURES) {
			LUM_LOG_ERROR("Max textures reached");
		...
		glTextureParameteri(texture.handle.gl_handle, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTextureParameteri(texture.handle.gl_handle, GL_TEXTURE_WRAP_T, GL_REPEAT);
