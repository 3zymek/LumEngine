...
namespace settings {
	...
	inline constexpr unsigned int MAX_POOL_CAPACITY = 1'048'576;
	inline constexpr unsigned int MAX_ENTITY_COUNT	= 1'048'576;

	// Defines how much containers needs to be reserved for components
	inline constexpr unsigned int MAX_COMPONENT_TYPES_COUNT = 16;
	...
	inline constexpr unsigned int MAX_SOUNDS_COUNT		= 1024;
	inline constexpr unsigned int MAX_CHANNELS_COUNT	= 512;
	...
	inline constexpr uint32_t MAX_EVENT_TYPES			= 32;

	// (All settings below are for each type of event)

	inline constexpr uint32_t MAX_EVENTS_PER_FRAME		= 1024;
	inline constexpr uint32_t MAX_CALLBACKS_PER_FRAME	= 32;
	inline constexpr uint32_t MAX_PERM_CALLBACKS		= 32;
