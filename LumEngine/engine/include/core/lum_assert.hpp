...
#define LUM_HOTCHK(expr, msg, ...) \
        do { if(!(expr)) { LUM_LOG_WARN(msg, ##__VA_ARGS__); } } while(0)

#define LUM_HOTCHK_RETURN_VOID(expr, msg, ...) \
        do { if (!(expr)) { LUM_LOG_WARN(msg, ##__VA_ARGS__); return; } } while (0)

#define LUM_HOTCHK_RETURN_NPTR(expr, msg, ...) \
        do { if (!(expr)) { LUM_LOG_WARN(msg, ##__VA_ARGS__); return nullptr; } } while (0)

#define LUM_HOTCHK_RETURN_0(expr, msg, ...) \
        do { if (!(expr)) { LUM_LOG_WARN(msg, ##__VA_ARGS__); return 0; } } while (0)

#define LUM_HOTCHK_RETURN_CUSTOM(expr, retival, msg, ...) \
        do { if(!(expr)) { LUM_LOG_WARN(msg, ##__VA_ARGS__); return retival; } } while(0)
